...
- **skills-lock.json** — check for `ui-ux-pro-max`; apply when the request involves professional audits or tier-1 standards
...
## Professional audit leverage (ui-ux-pro-max)

When the user requests professional UI/UX audits, tier-1 Web3 standards, "delta" improvements, or structural refinements (navbar cognitive load, layout-shift prevention, bento consistency, legal typography, breadcrumbs, exit-intent, skeleton states), also apply the **ui-ux-pro-max** skill from `skills-lock.json`. Use it to elevate from functional to Tier-1 product quality.
