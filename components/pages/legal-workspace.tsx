...
const docSectionClass =
  "w-full max-w-[min(100%,48rem)] mx-auto animate-slide-up px-4 sm:px-6 lg:px-10 py-6 sm:py-8 lg:py-12";

export function LegalWorkspace() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const [activePage, setActivePage] = useState<LegalViewId>(() =>
    viewFromSearchParam(searchParams.get("view")),
  );
  const [mobileNavOpen, setMobileNavOpen] = useState(false);

  useEffect(() => {
    setActivePage(viewFromSearchParam(searchParams.get("view")));
  }, [searchParams]);
  ...
  const setView = useCallback(
    (id: LegalViewId) => {
      ...
      router.replace(`/legal?view=${searchParamFromView(id)}`, { scroll: false });
    },
    [router],
  );
  ...
  const renderPage = () => {
    switch (activePage) {
      case 'legal':
        return <LegalHub onPageChange={setView} />;
      case 'changelog':
        return <Changelog />;
      case 'TeamRoles':
        return <Career />;
        ...
        return (
          <section className={docSectionClass}>
            ...
            <div className="mb-12 border-b border-white/5 pb-8">
              <h1 className="text-3xl font-medium tracking-tight text-white sm:text-4xl lg:text-5xl mb-4">
                Privacy Policy
              </h1>
...
contact us at:</p>
              <div className="rounded-xl border border-white/5 bg-white/[0.02] p-4 sm:p-8 flex flex-col gap-6">
                  {/* Email */}
                  <div className="flex flex-col gap-2 text-gray-300 sm:flex-row sm:items-center sm:gap-3">
                  ...
                  {/* Website */}
                  <div className="flex flex-col gap-2 text-gray-300 sm:flex-row sm:items-center sm:gap-3">
                    <Globe className="h-5 w-5 shrink-0 opacity-80" />

                    <span className="font-semibold text-white sm:shrink-0">Website:</span>
                    <a
...
Service after the updated Terms become effective constitutes your acceptance of them.</p>
  ...
  return (
    <div className="relative w-full overflow-x-hidden bg-slate-950 text-slate-400 antialiased selection:bg-violet-500/30 selection:text-white">
      <BackgroundEffects />

      {/* Mobile: doc title + open sidebar */}
      <div className="sticky top-[var(--nav-height)] z-30 flex items-center gap-3 border-b border-white/5 bg-slate-950/90 px-4 py-3 backdrop-blur-md lg:hidden">
        <button
          type="button"
          onClick={() => setMobileNavOpen(true)}
          className="inline-flex h-11 min-w-[44px] items-center justify-center rounded-lg border border-white/10 bg-white/5 text-white hover:bg-white/10"
          aria-expanded={mobileNavOpen}
          aria-controls="legal-sidebar-nav"
        >
          <Menu className="h-5 w-5" aria-hidden />
          <span className="sr-only">Open legal menu</span>
        </button>
        <p className="min-w-0 flex-1 truncate text-sm font-medium text-white">{LEGAL_VIEW_LABEL[activePage]}</p>
      </div>

      <div className="mx-auto flex w-full max-w-[1600px] flex-col lg:flex-row lg:items-start lg:gap-2">
        <Sidebar
          activePage={activePage}
          onPageChange={setView}
        ...
        <main
          id="legal-main"
          className="min-w-0 flex-1 px-0 pb-12 pt-2 sm:px-2 lg:px-4 lg:pb-16 lg:pt-4"
        >
          <div className="mx-auto w-full max-w-[min(100vw,56rem)]">{renderPage()}</div>
        </main>
      </div>
    </div>
  );
