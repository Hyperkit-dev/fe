...
export default function MagicRings({
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      const rw = Math.max(1, w * dpr);
      const rh = Math.max(1, h * dpr);
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      mouseRef.current[0] = (e.clientX - rect.left) / rw - 0.5;
      mouseRef.current[1] = -((e.clientY - rect.top) / rh - 0.5);
    };
    const onMouseEnter = () => {
    ...
    const animate = (t: number) => {
      ...
      const speedEff = Math.max(p.speed, 0.0001);
      ...
      uniforms.uTime.value = t * 0.001 * speedEff;
      ...
      uniforms.uRingCount.value = Math.min(10, Math.max(1, Math.floor(p.ringCount)));
